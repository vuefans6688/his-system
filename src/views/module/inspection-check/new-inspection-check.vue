<template>
  <div class="new-inspection-check">
    <div class="header">
    ...
    <el-form :model="form" ref="form" size="small" label-width="90px">
      ...
      <div v-for="(item, index) in form.data.item_list" :key="index" class="content">
        ...
        <!-- 药品名称 -->
        <el-form-item label="药品名称:" :rules="{required: true, message: '项目名称不能为空', trigger: 'blur, change'}">
          <el-autocomplete popper-class="my-autocomplete" v-model="item.name" placeholder="请输入名称"
            :fetch-suggestions="querySearchAsync" :trigger-on-focus="false" value-key="name"
            @select="inspectionSelect($event, index)" style="width: 100%;">
            <template slot-scope="props">
              <div class="think">
                <span class="name">{{ props.item.name }}</span>
                <span class="spec">{{ `${props.item.price}元` }}</span>
              </div>
            </template>
          </el-autocomplete>
        </el-form-item>
        <!-- 数量 -->
        <el-form-item label="数量:" :rules="{ required: true, message: '数量不能为空', trigger: 'blur, change' }">
          <el-input v-model="item.total" @keyup.native="numberInput('total')" @change="calculateAll(index)"></el-input>
        </el-form-item>
        <!-- 数量单位 -->
        <el-form-item label="数量单位:" :rules="{ required: true, message: '单位不能为空', trigger: 'blur, change' }">
          <el-input placeholder="请输入数量单位" v-model="item.total_unit" :maxlength="64"></el-input>
        </el-form-item>
        <!-- 单价 -->
        <el-form-item label="单价(元):">
          <span style="color: #f00;">￥ {{ item.showprice | priceFilter }}</span>
        </el-form-item>
        <!-- 金额 -->
        <el-form-item label="金额(元):">
          <span style="color: #f00;">￥ {{ item.total_showprice | priceFilter }}</span>
        </el-form-item>
        <!-- 操作 -->
...
export default {
  ...
  methods: {
    ...
    inspectionSelect(item, index) {
      ...
      this.form.data.item_list[index].showprice = item.price
      this.form.data.item_list[index].total_showprice = item.total ? parseFloat(item.showprice * item.total).toFixed(2) : ''
      const newData = JSON.stringify(this.form)
      this.form = []
      this.form = JSON.parse(newData)
      this.calculateAll(index)
    },
    calculateAll(index) {
      const item = this.form.data.item_list[index]
      if (!item.item_id) {
        this.$message.warning({
          message: '请先从项目选框中选择诊疗项目名称',
          duration: 2000,
          showClose: true
        })
        return
      ...
      item.total_showprice = parseFloat(item.showprice * item.total)
      // 解决数据改变界面不渲染的问题
      const newData = JSON.stringify(this.form)
      this.form = []
      this.form = JSON.parse(newData)
    ...
    stringNumber() {
      this.form.data.item_list.forEach((item, index) => {
        this.form.data.item_list[index].total = item.total ? parseFloat(item.total) : 0
      })
    }
  },
  computed: {
